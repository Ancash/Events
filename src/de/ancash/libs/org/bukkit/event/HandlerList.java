...
public class HandlerList {
	...
	private final CopyOnWriteArrayList<HandlerList> children = new CopyOnWriteArrayList<HandlerList>(); // Not modified
																										// that much,
																										// it's fine
	...
	/**
	 * Create a new handler list and initialize using EventPriority The HandlerList
	 * is then added to meta-list for use in bakeAll()
	 */
	...
	public void register(ListenerRegistration listener) {
		if (handlerSlots.get(listener.getOrder()).contains(listener)) {
			throw new IllegalStateException(
					"This listener is already registered to priority " + listener.getOrder().toString());
		}
		dirty();
